...
- (void)reportStatus
{
    ...
    TDLog(@"Finished prefetching (%d successful, %d skipped, timeElasped %.2f)", total - _skippedCount, _skippedCount, CFAbsoluteTimeGetCurrent() - _startedTime);
}
...
{
    _finishedCount++;
    TDLog(@"Prefetched %d out of %d", _finishedCount, [self.prefetchURLs count]);
...
{
    _finishedCount++;
    TDLog(@"Prefetched %d out of %d (Failed)", _finishedCount, [self.prefetchURLs count]);
